...
(defun list-hand        () (mapcar (lambda (x) (format t "~a~%" (get-card-name (eval x)))) (get-player-hand *active-player*)))
(defun list-battlefield () (mapcar (lambda (x) (format t "~a~%" (get-card-name (eval x)))) *battlefield*))
(defun list-graveyard   () (mapcar (lambda (x) (format t "~a~%" (get-card-name (eval x)))) *graveyard*))
(defun list-exile       () (mapcar (lambda (x) (format t "~a~%" (get-card-name (eval x)))) *exile*))
...
(defun play-card (inname)
        (let* ((handcards (get-player-hand *active-player*))
               (c (find-if (lambda (x) (equalp inname (get-card-name (eval x)))) handcards)))
                (if c
		    (if (land? (eval c))
			(progn (format t "playing ~a~%" (get-card-name (eval c)))
			       (setf *battlefield* (cons (eval c) *battlefield*))
...
(defun cast-card (inname)
        (let* ((handcards (get-player-hand *active-player*))
			...
			(progn (format t "casting ~a~%" (get-card-name (eval c)))
			       (setf *battlefield* (cons (eval c) *battlefield*))
			       (set-player-hand *active-player* (remove c handcards :count 1))))
...
(defun describe-card (inname)
        (let1 c (find-if (lambda (x) (equalp inname (get-card-name (eval x)))) *all-cards*)
                (if c
                    (print (eval c))
