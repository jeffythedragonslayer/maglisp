...
(defun list-hand        () (mapcar (lambda (x) (format t "~a~%" (card-name (eval x)))) (objs (hand *active-player*)))) 
(defun list-battlefield () (mapcar (lambda (x) (format t "~a~%" (card-name (eval x)))) (objs *mtg-battlefield*))) 
(defun list-graveyard   () (mapcar (lambda (x) (format t "~a~%" (card-name (eval x)))) (objs *mtg-graveyard*))) 
(defun list-exile       () (mapcar (lambda (x) (format t "~a~%" (card-name (eval x)))) (objs *mtg-exile*))) 
...
(defun play-card (inname)
        (let* ((handcards (objs (hand *active-player*)))
               (c (find-if (lambda (x) (equalp inname (card-name (eval x)))) handcards)))
                (if c
                    (progn (format t "playing ~a~%" (card-name (eval c)))
                           (setf (objs *mtg-battlefield*) (cons (eval c) (objs *mtg-battlefield*)))
                           (setf (objs (hand *active-player*)) (remove c handcards :count 1))) 
...
(defun cast-card (inname)
        (let* ((handcards (objs (hand *active-player*)))
               (c (find-if (lambda (x) (equalp inname (card-name (eval x)))) handcards)))
                (if c
                    (progn (format t "casting ~a~%" (card-name (eval c)))
                           (setf (objs *mtg-battlefield*) (cons (eval c) (objs *mtg-battlefield*)))
                           (setf (objs (hand *active-player*)) (remove c handcards :count 1))) 
                    (format t "not a card in your card~%"))))

(defun tap-card (bnum) nil)

(defun describe-card (inname)
        (let1 c (find-if (lambda (x) (equalp inname (card-name (eval x)))) *all-cards*)
                (if c
                    (print (eval c))
