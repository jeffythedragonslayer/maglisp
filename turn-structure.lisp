			      ...
			      (format t "~a has summoning sickness, can't attack this turn~%" (get-card-name c))
 			      (if (tapped? c)
 			 	  (format t "~a tapped, can't attack~%" (get-card-name c))
                                  (if (member c *declared-attackers*)
                                      (format t "already attacking with ~a~%" (get-card-name c))
                                      (progn (format t "going to attack with ~a~%" (get-card-name c))
			  ...
			  (if (sick? c)
			      (format t "~a has summoning sickness, can't attack this turn~%" (get-card-name c))
 			      (if (tapped? c)
 			 	  (format t "~a tapped, can't attack~%" (get-card-name c))
                                  (if (member c *declared-attackers*)
                                      (progn (format t "not going to attack with ~a~%" (get-card-name c))
                                             (setf *declared-attackers* (remove c *declared-attackers*)))
                                      (format t "already not attacking with ~a~%" (get-card-name c)))))
			  (format t "~a not a creature, can't attack~%" (get-card-name c))))
                (condition () (format t "enter an integer between 0 and battlefieldsize-1~%"))))
