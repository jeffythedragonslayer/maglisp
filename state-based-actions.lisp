...
(defun phased-token-die              () "blah") ;704.5d If a token is phased out, or is in a zone other than the battlefield, it ceases to exist.  
...
(defun too-many-counters             () "blah") 
(defun world-rule                    () "blah") ;704.5m If two or more permanents have the supertype world,
...
(defun illegal-aura-die              () "blah") ;704.5n If an Aura is attached to an illegal object or player, or is not attached to an object or player, that Aura is put into its owner’s graveyard.  
(defun illegal-equipment-die         () "blah") ;704.5p If an Equipment or Fortification is attached to an illegal permanent, it becomes unattached from that permanent. It remains on the battlefield.  
(defun illegal-creature-attachment   () "blah") ;704.5q If a creature is attached to an object or player, it becomes unattached and remains on the battlefield. Similarly,
...
(defun up-down-counters-cancel       () "blah") ;704.5r If a permanent has both a +1/+1 counter and a -1/-1 counter on it, N +1/+1 and N -1/-1 counters are removed from it,
                                                ; where N is the smaller of the number of +1/+1 and -1/-1 counters on it.  
(defun illegal-counters              () "blah") ;704.5s If a permanent with an ability that says it can’t have more than N counters of a certain kind on it
...
(defun check-state-based-actions ()
        (mapcar (lambda (x) (funcall (eval x))) *state-based-actions*))
